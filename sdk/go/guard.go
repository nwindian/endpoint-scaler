// Package microapi provides middleware for endpoint-scaler controlled routing.
package microapi
...
const (
	...
	GuardrailEnvVar = "MICROAPI_GUARDRAIL"
)

// Guard wraps an HTTP handler and only executes it if the MICROAPI_GUARDRAIL
...
// has MICROAPI_GUARDRAIL set to its endpoint ID. This ensures that only the
// appropriate handler processes requests for that endpoint.
//
// Usage:
//
//	mux := http.NewServeMux()
//	mux.Handle("/lookup", microapi.Guard("lookup", lookupHandler))
//	mux.Handle("/search", microapi.Guard("search", searchHandler))
//
// When MICROAPI_GUARDRAIL is not set (e.g., in development), all handlers are active.
// When MICROAPI_GUARDRAIL is set to "lookup", only the lookup handler processes requests.
...
// Usage:
//
//	if microapi.IsActiveEndpoint("lookup") {
...
// requests when the MICROAPI_GUARDRAIL matches the endpoint ID.
//
// Usage with gRPC:
//
//	server := grpc.NewServer(
//	    grpc.UnaryInterceptor(microapi.GRPCUnaryInterceptor("lookup")),
//	)
//
